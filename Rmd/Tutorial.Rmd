...
Below, you can see the rank-abundance distribution from one of the catchments in our Turkey Lakes benthic invertebrate dataset from the year following logging treatments.
...
In the above plot, we can compare species richness by comparing the maximum rank.

```{r}
df %>% 
  filter(TotalCount > 0) %>%
  summarise(maxRank = max(Rank)) -> annotations
