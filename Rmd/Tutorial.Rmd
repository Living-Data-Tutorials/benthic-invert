...
An experiment was conducted in 1997, where the forests next to the sampling sites were logged with differing intensities.

Streamside logging affects many stream variables such as temperature, light exposure, leaf litter deposition, and algae growth. Because benthic invertebrates are affected by these variables, we can hypothesize that the biodiversity of benthic invertebrate communities might be affected by logging.
...
Once we have collected our biodiversity data, there are several different ways to analyze and understand changes in community composition. Each of these metrics allow us to look at different aspects of the biodiversity data we collect.
...
# Table showing the number of unique species
...
shannon<-  sim %>%
    ...
    summarise(n=n()) %>%
    mutate(prop = n/sum(n),
    shannon= sum(prop/log(prop))) %>% # Calculate Shannon's index
...
simpson<-  sim %>%
    ...
    mutate(prop = n/sum(n),
    simpson = sum((prop)^2)) %>%
    summarise(simpson = mean(simpson)) # Calculate Simpson's index

kable(simpson, col.names=c("Year", "Simpson's Index"), align="c")%>%
  kable_classic(full_width = F, html_font = "Cambria")
```
Interestingly, the Simpson index appears to increase over time. This is likely because Simpson's index weights the more dominant species higher, so the value can actually increase even when biodiversity is decreasing.
